...
export async function handleCallbackQuery(callbackQuery) {
  ...
  try {
    ...
    if (data.startsWith('panic_ack:')) {
      const panicId = data.replace('panic_ack:', '');
      const operatorName = `@${from.username || from.first_name || 'Operador'}`;

      await acknowledgePanic(panicId, operatorName);
      await answerCallbackQuery(callbackQueryId, '✅ Alerta de pánico reconocida');

      if (chatId && message?.message_id) {
        await editMessageText(
          chatId,
          message.message_id,
          `${message.text}\n\n✅ <b>RECONOCIDO POR:</b> ${operatorName} a las ${new Date().toLocaleTimeString('es-CL')}`
        );
      }
    } else if (data.startsWith('panic_resolve:')) {
      const panicId = data.replace('panic_resolve:', '');
      const operatorName = `@${from.username || from.first_name || 'Operador'}`;

      await resolvePanic(panicId, operatorName, 'Resuelto desde Telegram');
      await answerCallbackQuery(callbackQueryId, '✔️ Alerta de pánico resuelta');

      if (chatId && message?.message_id) {
        await editMessageText(
          chatId,
          message.message_id,
          `${message.text}\n\n✔️ <b>RESUELTO POR:</b> ${operatorName} a las ${new Date().toLocaleTimeString('es-CL')}`
        );
      }
    } else {
      await answerCallbackQuery(callbackQueryId, 'Acción procesada');
    }
  } catch (err) {
    console.error('[handleCallbackQuery] Error:', err.message);
    await answerCallbackQuery(callbackQueryId, 'Error al procesar acción');
  }
}
